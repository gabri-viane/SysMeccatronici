...
void treTratti(Instructions &ci, Servo &s) {
	...
	auto M = getMatLAB();
#endif
	unsigned int durata_secondi = ci.tempo_tot_ms / 1000;  // Converto da ms a s perché le formule sono in secondi
	...
	double durate[] = { durata_secondi * ci.lambdas.cost_1,
					  durata_secondi * ci.lambdas.cost_2,
					  durata_secondi * ci.lambdas.cost_3 };
	// creo una variabile di supporto per non effettuare più volte lo stesso calcolo
	double tmp = ci.delta_angolo * 2 / durata_secondi;
	// ricavo la velocità massima che raggiungo
	double v_max = tmp / (2 - ci.lambdas.cost_1 - ci.lambdas.cost_3);
	...
	double tmp2 = 2 - ci.lambdas.cost_1 - ci.lambdas.cost_3;

	// Calcolo le accelerazioni A+ e A-
	double acc1 = tmp / (ci.lambdas.cost_1 * tmp2);
	double acc3 = tmp / (ci.lambdas.cost_3 * tmp2);
